...
const seams = [
  ...
  {
    id: 'server-csp-worker-src',
    file: 'server/cli/server.ts',
    ref: 'H13',
    description: 'CSP worker-src pinned to self (no workers used; do not inherit script-src breadth)',
    applied: `'worker-src': ['\\'self\\'']`,
    op: 'insert-after',
    anchor: `'form-action': [`,
    lines: [
  ...
  {
    id: 'server-index-nocache',
    file: 'server/cli/server.ts',
    ref: 'H13',
    description: 'no-cache on the served index.html route',
    applied: `app.get(\`\${basePath()}/index.html\`, noCache`,
    op: 'replace-line',
    match: `app.get(\`\${basePath()}/index.html\`, (_req, res) => {`,
    replacement: `  app.get(\`\${basePath()}/index.html\`, noCache, (_req, res) => {`,
  },
  {
    id: 'server-fallback-nocache',
    file: 'server/cli/server.ts',
    ref: 'H13',
    description: 'no-cache on SPA fallback responses',
    applied: `// fork-seam: no-cache on SPA fallback responses`,
    op: 'insert-before',
  ...
  // ---------- server/ldap/server.ts ----------
  {
    id: 'ldap-import-hardening',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'import fork-owned LDAP hardening module',
    applied: `from './hardening'`,
    op: 'insert-after',
    anchor: `import { als } from '../util/als'`,
    lines: [
  ...
  {
    id: 'ldap-connection-guard',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'connection cap and idle timeout on new LDAP connections',
    applied: `if (!admitConnection(socket))`,
    op: 'insert-after',
    anchor: `function handleConnection(socket: Socket) {`,
    lines: [
  ...
  {
    id: 'ldap-bind-blocked-check',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'refuse binds from sources under failure backoff',
    applied: `if (isBindBlocked(connection.socket))`,
    op: 'insert-after',
    anchor: `const password = auth.tag === 0x80 ? auth.value.toString('utf8') : ''`,
    lines: [
      ``,
  ...
  {
    id: 'ldap-record-success-binddn',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'record successful service bind',
    applied: `// fork-seam: record successful service bind`,
    op: 'insert-after',
    anchor: `logger({ level: 'debug', message: 'LDAP bind request with valid bind DN and password' })`,
    lines: [
  ...
  {
    id: 'ldap-record-failure-binddn',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'record failed service bind',
    applied: `// fork-seam: record failed service bind`,
    op: 'insert-after',
    anchor: `logger({ level: 'debug', message: 'LDAP bind request with invalid bind DN or password' })`,
    lines: [
  ...
  {
    id: 'ldap-record-failure-user',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'record failed user bind',
    applied: `// fork-seam: record failed user bind`,
    op: 'insert-after',
    anchor: `logger({ level: 'debug', message: 'LDAP bind request with invalid user credentials' })`,
    lines: [
  ...
  {
    id: 'ldap-record-failure-canlogin',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'record bind of a login-ineligible user',
    applied: `// fork-seam: record bind of a login-ineligible user`,
    op: 'insert-after',
    anchor: `logger({ level: 'debug', message: 'LDAP bind request with user unable to login with a password' })`,
    lines: [
  ...
  {
    id: 'ldap-record-success-user',
    file: 'server/ldap/server.ts',
    ref: 'H2',
    description: 'record successful user bind',
    applied: `// fork-seam: record successful user bind`,
    op: 'insert-after',
    anchor: `logger({ level: 'debug', message: 'LDAP bind request with valid user credentials' })`,
    lines: [
